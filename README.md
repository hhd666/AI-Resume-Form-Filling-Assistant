# AI 简历填表助手
## fork自 https://github.com/1lck/AI-Resume-Form-Filling-Assistant
### 根据我自己在投简历过程中遇到的问题进行了补充。

## 改进功能如下：
1. 新增AI思考时间显示
2. 原先的简历存储机制会导致简历字段过多时，无法保存新的简历内容。把chrome.stroge.sync改成chrome.stroge.local
3. 简历导入导出到json文件，仅本插件用
4. 简历内容加入“专利/论文”经历，获奖经历单独分类
5. 精简输入AI的数据，包括简历字段精简和扫描表单精简，输入AI数据Kb降低60%，100kB输入数据降低到40kB左右，节省TOKEN。
6. 适配飞书、MOKAHR、北森的日期选择控件、文本输入控件
7. 选区填入改成选点，选择左上角后，可以任意滚动网页，再选择右下角，实现长页面填写。原先的选区只能选一页高度，不知道是不是我操作不对？
8. 侧边栏加入个人简历预览，现在要手动填简历打开侧边栏可以选择“转写”，直接写入选择的输入框。
